...
void AudioExtractor::Process()
{
    ...
    {
        if (!StringUtils::EndsWith(itr.first, ".wav") && !StringUtils::EndsWith(itr.first, ".ogg")
            && !StringUtils::EndsWith(itr.first, ".wav.ogg") && !StringUtils::EndsWith(itr.first, ".mp3"))
            continue;
    ...
    enki::TaskSet convertAudioTask(numFiles, [&](enki::TaskSetPartition range, uint32_t threadNum)
    {
        for (u32 i = range.start; i < range.end; i++)
        {
            ...
            audioConvert.ConvertAudio(buffer->GetDataPointer(), buffer->writtenData, fileListEntry.path);
